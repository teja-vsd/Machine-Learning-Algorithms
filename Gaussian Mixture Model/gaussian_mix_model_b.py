# -*- coding: utf-8 -*-
"""
Created on Mon Jun 12 04:21:44 2017
...
# -*- coding: utf-8 -*-
"""
Created on Mon Jun 12 03:24:24 2017
...
def posterior_denom(likelihood_elm,prior_prob):
    #post_denom = np.zeros(np.shape(likelihood_mat)[0])
...
def posterior_random(n,k):
    ...
    for i in range(n):
        ...
        for j in range(k):
            if j == 0:
                posterior[i][j] = np.random.uniform(0,1)
                sm += posterior[i][j]
            elif j>0 and j!=k-1:
                posterior[i][j] = np.random.uniform(0,1-sm)
                sm += posterior[i][j]
            else:
                posterior[i][j] = 1-sm
...
posterior_mat = posterior_random(np.shape(dataset)[0],k)
print(posterior_mat)
